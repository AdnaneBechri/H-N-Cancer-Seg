...
class InputTransition(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.bn1(self.conv1(x2))
        # split input in to 16 channels (or is it right to say duplicate the input for 16 times to operate "torch.add()"?? By Chao)
        x161 = torch.cat((x1, x1, x1, x1, x1, x1, x1, x1), 1) # changed dim = 0 to 1 to operate on channels, and have "x16" the same size as "out" to operate "torch.add()". By Chao.
        x162 = torch.cat((x2, x2, x2, x2, x2, x2, x2, x2), 1)
        # print("x16 shape:"+str(x16.shape))
        # pdb.set_trace()
        out1 = self.relu1(torch.add(out1, x161))
        out2 = self.relu1(torch.add(out2, x162))
