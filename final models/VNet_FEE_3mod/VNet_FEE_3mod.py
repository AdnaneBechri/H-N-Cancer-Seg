...
class InputTransition(nn.Module):
    def __init__(self, outChans, elu):
        ...
        self.conv1 = nn.Conv3d(1, 5, kernel_size=5, padding=2)
        self.bn1 = ContBatchNorm3d(5)
        self.relu1 = ELUCons(elu, 5)

        self.conv2 = nn.Conv3d(1, 6, kernel_size=5, padding=2)
        self.bn2 = ContBatchNorm3d(6)
        self.relu2 = ELUCons(elu, 6)
    ...
    def forward(self, x):
        ...
        out2 = self.bn2(self.conv2(x2))
        out3 = self.bn1(self.conv1(x3))
        # split input in to 16 channels (or is it right to say duplicate the input for 16 times to operate "torch.add()"?? By Chao)
        x161 = torch.cat((x1, x1, x1, x1, x1), 1) # changed dim = 0 to 1 to operate on channels, and have "x16" the same size as "out" to operate "torch.add()". By Chao.
        x162 = torch.cat((x2, x2, x2, x2, x2, x2), 1)
        x163 = torch.cat((x3, x3, x3, x3, x3), 1)
        #print("x16 shape:"+str(x161.shape))
        # pdb.set_trace()
        out1 = self.relu1(torch.add(out1, x161))
        out2 = self.relu2(torch.add(out2, x162))
        out3 = self.relu1(torch.add(out3, x163))
...
class VNet_FEE(nn.Module):
    # the number of convolutions in each layer corresponds
    # to what is in the actual prototxt, not the intent
    def __init__(self, elu=True, nll=False):
        super(VNet_FEE, self).__init__()
