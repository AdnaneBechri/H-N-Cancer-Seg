...
class UNet_FED(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters):
        ...
        # Up sampling
        self.trans_1 = conv_trans_block_3d(self.num_filters * 64, self.num_filters * 32, activation)
    ...
    def forward(self, x):
        ...
        bridge  = torch.cat([bridge1, bridge2], dim=1)
        # Up sampling

        trans_1 = self.trans_1(bridge)  # -> [1, 128, 8, 8, 8]
        concat_1 = torch.cat([trans_1, torch.add(down_51,down_52)], dim=1)  # -> [1, 192, 8, 8, 8]
        up_1 = self.up_1(concat_1)  # -> [1, 64, 8, 8, 8]

        trans_2 = self.trans_2(up_1)  # -> [1, 64, 16, 16, 16]
        concat_2 = torch.cat([trans_2, torch.add(down_41,down_42)], dim=1)  # -> [1, 96, 16, 16, 16]
        up_2 = self.up_2(concat_2)  # -> [1, 32, 16, 16, 16]

        trans_3 = self.trans_3(up_2)  # -> [1, 32, 32, 32, 32]
        concat_3 = torch.cat([trans_3, torch.add(down_31,down_32)], dim=1)  # -> [1, 48, 32, 32, 32]
        up_3 = self.up_3(concat_3)  # -> [1, 16, 32, 32, 32]

        trans_4 = self.trans_4(up_3)  # -> [1, 16, 64, 64, 64]
        concat_4 = torch.cat([trans_4, torch.add(down_21,down_22)], dim=1)  # -> [1, 24, 64, 64, 64]
        up_4 = self.up_4(concat_4)  # -> [1, 8, 64, 64, 64]

        trans_5 = self.trans_5(up_4)  # -> [1, 8, 128, 128, 128]
        concat_5 = torch.cat([trans_5, torch.add(down_11,down_12)], dim=1)  # -> [1, 12, 128, 128, 128]
