...
class VNet_FED(nn.Module):
    ...
    def forward(self, x):
        ...
        outp = torch.cat((out2561, out2562), 1)

        out = self.up_tr256(outp, torch.add(out1281,out1282))
        out = self.up_tr128(out, torch.add(out641,out642))
        out = self.up_tr64(out, torch.add(out321,out322))
        out = self.up_tr32(out, torch.add(out161,out162))
        out = self.out_tr(out)
        """"
