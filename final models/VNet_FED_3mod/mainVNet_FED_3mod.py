...
def inferencee(network, x_train, y_train, imageNames, epoch, folder_save, number_modalities):
    ...
    for i_p in range(x_train.shape[0]):
        pred = network(
            numpy_to_var(x_train[i_p, :, :, :, :].reshape(1, 3, patchSize, patchSize, patchSize)))
        pred_y = softMax(pred)
...
def inference(network, moda_n, moda_g, imageNames, epoch, folder_save, number_modalities):
    ...
    for i_s in range(len(imageNames)):
        if number_modalities == 2:
            ...
            patch_1, patch_2, patch_3, patch_g, img_shape = load_data_test(moda_n, moda_g, imageNames[i_s],
                                                                           number_modalities)  # hardcoded to read the first file. Loop this to get all files
        ...
        dsc = dc(gt, bin_seg)
        acc = accuracy_score(gt.flatten(), bin_seg.flatten())
        dscAll.append(dsc)
        accall.append(acc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelName', type=str, default='VNet_FED_3mod', help='name of the model')
    ...
    parser.add_argument('--numEpochs', type=int, default=201, help='Number of epochs')
    parser.add_argument('--batchSize', type=int, default=4, help='Batch size')
    parser.add_argument('--l_rate', type=float, default=0.0004, help='Learning rate')
