...
class InputTransition(nn.Module):
    ...
    def forward(self, x):
        ...
        x161 = torch.cat((x, x, x, x, x,x,x,x,x,x, x, x, x,x,x,x), 1) # changed dim = 0 to 1 to operate on channels, and have "x16" the same size as "out" to operate "torch.add()". By Chao.

        #print("x16 shape:"+str(x161.shape))
        # pdb.set_trace()
        out = self.relu1(torch.add(out1, x161))
...
class DownTransition1(nn.Module):
    def __init__(self, inChans, nConvs, elu, dropout=False):
        super(DownTransition1, self).__init__()
        outChans = inChans
        self.down_conv = nn.Conv3d(inChans, outChans, kernel_size=2, stride=2)
        self.bn1 = ContBatchNorm3d(outChans)
        self.do1 = passthrough
...
class VNet_FED(nn.Module):
    # the number of convolutions in each layer corresponds
    # to what is in the actual prototxt, not the intent
    def __init__(self, elu=True, nll=False):
        super(VNet_FED, self).__init__()
        self.in_tr = InputTransition(16, elu)
        self.down_tr32 = DownTransition(16, 1, elu)
        self.down_tr64 = DownTransition(32, 2, elu)
        self.down_tr128 = DownTransition(64, 3, elu, dropout=True)
        self.down_tr256 = DownTransition(128, 2, elu, dropout=True)
        self.up_tr256 = UpTransition(768, 256, 2, elu, dropout=True)
    ...
    def forward(self, x):
        ...
        out161 = self.in_tr(x[:,0:1,:,:,:])
        out321 = self.down_tr32(out161)
        out641 = self.down_tr64(out321)
        out1281 = self.down_tr128(out641)
        out2561 = self.down_tr256(out1281)

        out162 = self.in_tr(x[:,1:2,:,:,:])
        out322 = self.down_tr32(out162)
        out642 = self.down_tr64(out322)
        out1282 = self.down_tr128(out642)
        out2562 = self.down_tr256(out1282)

        out163 = self.in_tr(x[:, 2:3, :, :, :])
        out323 = self.down_tr32(out163)
        out643 = self.down_tr64(out323)
        out1283 = self.down_tr128(out643)
        out2563 = self.down_tr256(out1283)


        outp = torch.cat((out2561, out2562, out2563), 1)

        out = self.up_tr256(outp, torch.add(torch.add(out1281,out1282), out1283))
        out = self.up_tr128(out, torch.add(torch.add(out641,out642), out643))
        out = self.up_tr64(out, torch.add(torch.add(out321,out322), out323))
        out = self.up_tr32(out, torch.add(torch.add(out161,out162), out162))
        out = self.out_tr(out)
        """"
